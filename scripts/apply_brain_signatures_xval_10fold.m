...
%     {'CS+rev', 'CS-rev'}, {'CS+revearly', 'CS+revlate', 'CS-revearly', 'CS-revlate'}};
contrast_names = {{'CS+', 'CS-'}, {'CS+>CS-'}, {'CS+early', 'CS+late', 'CS-early', 'CS-late'}, ...
    {'newCS+', 'newCS-'}, {'CS+revearly', 'CS+revlate', 'CS-revearly', 'CS-revlate'}};

metric = 'dot_product'; % dot_product cosine_similarity
%% Load brain signatures
load(fullfile(save_results, 'stats_CV_10fold.mat'));
stats_CV_10f = stats_CV_10f2;
load(fullfile(save_results, 'training_data.mat'));
load(fullfile(save_results, '10fold_CV.mat'));
...
for s = 1:length(subj_names)
    ...
    if tr_set(s) == 1
        ...
        sig_xval.dat = stats_CV_10f.other_output_cv{sample_folds(s_train, 1),1};
        comp_samplefolds(s_train) = sample_folds(s_train, 1);
        s_train = s_train + 1;
    else
        sig_xval = fmri_data(fullfile(save_results, 'svm_results_unthresholded_10foldCV.nii'), maskdir);
    end
...
% writetable(res_pat_exp, fullfile(save_results, 'pat_exp_late_all_data_xval_10fold.xlsx'), 'WriteRowNames', true);
...
for C = 1:length(contrast_files)
    ...
    title('Reversal Test set'); % Reversal Acquisition
...
figure;
r1 = roc_plot(stats_CV_10f.dist_from_hyperplane_xval, stats_CV_10f.Y == 1, ...
...
set(gca, 'FontSize', 28)
set(gcf,'position', [x0, y0, width, height])
saveas(gcf, fullfile(save_results, 'ROC_all_data_xval_10fold.png'))
